    ...
    public class LayoutAwarePage : Page
    {
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            else
            {
                ...
                // from cache
                this.LoadState(e.Parameter, frameState.ContainsKey(this._pageKey) ? (Dictionary<String, Object>)frameState[this._pageKey] : null);
            }
        }
        ...
        protected int LoadingItems
        {
            get
            {
                return DefaultViewModel.ContainsKey("LoadingItems") ? (int)DefaultViewModel["LoadingItems"] : 0;
            }
            set
            {
                int newLoadingItems = Math.Max(value, 0);
                if (newLoadingItems == LoadingItems)
                {
                    return;
                }
                DefaultViewModel["LoadingItems"] = newLoadingItems;
                IsLoading = newLoadingItems > 0;
            }
        }
        ...
        public bool IsLoading
        {
            get
            {
                return DefaultViewModel.ContainsKey("IsLoading") ? (bool)DefaultViewModel["IsLoading"] : false;
            }
            set
            {
                DefaultViewModel["IsLoading"] = value;
                if (!IsLoading)
                {
                    LoadingItems = 0;
                }
