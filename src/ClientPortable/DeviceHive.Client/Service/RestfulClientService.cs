...
namespace DeviceHive.Client
{
    ...
    public class RestfulClientService : IClientService
    {
        ...
        {
            return await GetAsync<List<Network>>("/network" + Query(filter));
        }
        ...
        {
            return await GetAsync<List<Device>>("/device" + Query(filter));
        }
        ...
        {
            return await GetNotificationsAsync(deviceId, new NotificationFilter { Start = start, End = end });
        }
        ...
        public async Task<List<Notification>> GetNotificationsAsync(Guid deviceId, NotificationFilter filter)
        {
            return await GetAsync<List<Notification>>(string.Format("/device/{0}/notification", deviceId) + Query(filter));
        }
        ...
        {
            return await GetCommandsAsync(deviceId, new CommandFilter { Start = start, End = end });
        }
        ...
        public async Task<List<Command>> GetCommandsAsync(Guid deviceId, CommandFilter filter)
        {
            return await GetAsync<List<Command>>(string.Format("/device/{0}/command", deviceId) + Query(filter));
        }
        ...
        {
            return await GetAsync<User>("/user/current");
        }
