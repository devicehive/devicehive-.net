...
namespace DeviceHive.Client
{
    ...
    public class RestfulClientService : IClientService, IDisposable
    {
        ...
        /// <summary>
        /// Gets access key used for service authentication.
        /// </summary>
        ...
        /// <summary>
        /// Uses access key for authentication.
        /// </summary>
        /// <param name="serviceUrl">URL of the DeviceHive service.</param>
        /// <param name="accessKey">Access key used for service authentication.</param>
        /// <param name="useWebSockets">Allow using WebSockets API (if it's available).</param>
        public RestfulClientService(string serviceUrl, string accessKey, bool useWebSockets = true)
        {
        ...
        /// <summary>
        /// Uses user credentials for authentication.
        /// </summary>
        /// <param name="serviceUrl">URL of the DeviceHive service.</param>
        ...
        /// <param name="useWebSockets">Allow using WebSockets API (if it's available).</param>
        public RestfulClientService(string serviceUrl, string login, string password, bool useWebSockets = true)
        {
            if (string.IsNullOrEmpty(serviceUrl))
                throw new ArgumentNullException("serviceUrl");
        ...
        /// <param name="timestamp">Last received notification timestamp.</param>
        /// <param name="token">Cancellation token used to cancel the polling operation.</param>
        /// <returns>A list of <see cref="DeviceNotification"/> objects.</returns>
        ...
        /// </summary>
        /// <param name="deviceIds">List of device unique identifiers. If empty - subscription for all available devices will be created</param>
        /// <remarks>
        ...
        /// </summary>
        /// <param name="deviceIds">List of device unique identifiers. If empty - subscription for all available devices will be removed</param>
        ...
        {
            var request = CreateWebRequest("GET", url);
            try
            {
        ...
        {
            var request = CreateWebRequest("POST", url);
        ...
        {
            var request = CreateWebRequest("PUT", url);
            using (var stream = request.GetRequestStream())
            {
                Serialize(stream, obj);
            }
        ...
        private void Delete<T>(string url)
        {
            var request = CreateWebRequest("DELETE", url);
            try
            {
                request.GetResponse();
            }
        ...
        private WebRequest CreateWebRequest(string method, string url)
        {
            Logger.DebugFormat("Calling {0} {1}", method, url);
